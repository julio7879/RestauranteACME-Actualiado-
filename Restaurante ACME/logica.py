...
def agregar_productos (productos):
    ...
    if existe == True:
        ...
        while True:
            ...
            if iva > 0:
                break
            print("Error el iva ingresado es negativo!")
        nuevo_p = {
            "codigo": codigo,
            "nombre":nombre,
            "vu": vu,
...
def crear_factura(productos, clientes, mesas, ventas):
    ...
    while True:
        ...
        if input("\n¿Desea agregar un nuevo producto? (s/n): ").lower() != "s":
            break
...
"""
    ...
    if input("¿Desea guardar esta factura? (s/n): ").lower() == "s":
        total_iva_factura = 0
        ...
        columnas_ventas = ["fecha", "mesa","cliente_id", "cliente_nombre", "total_productos","subtotal_bruto", "subtotal_iva", "total"]
        persistencia.guardar_archivo(ventas, "Ventas_ACME.csv", columnas_ventas)

        detalles_guardados = persistencia.cargar_datos("Detalle_Ventas_ACME.csv")
    ...
    columnas_detalle = ["fecha", "mesa", "codigo", "nombre", "cantidad", "vu", "subtotal"]
    persistencia.guardar_archivo(detalles_guardados, "Detalle_Ventas_ACME.csv", columnas_detalle)
...
def ranking_productos_menos_vendidos():
    detalles = persistencia.cargar_datos("Detalle_Ventas_ACME.csv")
    ...
    ranking = sorted(conteo.items(), key=lambda x: x[1]["cantidad"])

    print("\n--- TOP 5 PRODUCTOS MENOS VENDIDOS ---")
    for i, (codigo, info) in enumerate(ranking[:5], start=1):
        print(f"{i}. {info['nombre']} (Código: {codigo}) - Vendidos: {info['cantidad']}")
...
def ranking_clientes():
    ...
    for v in ventas:
        if fi <= v["fecha"] <= ff:
            cliente = v.get("cliente_nombre", "Desconocido")
            total = float(v["total"])

            compras[cliente] = compras.get(cliente, 0) + total

    if not compras:
        print("No hay datos en ese rango.")
        return

    ordenado = sorted(compras.items(), key=lambda x: x[1])

    print("\n--- CLIENTE QUE MENOS COMPRA ---")
    print(f"{ordenado[0][0]} - ${ordenado[0][1]:,.0f}")

    print("\n--- CLIENTE QUE MÁS COMPRA ---")
    print(f"{ordenado[-1][0]} - ${ordenado[-1][1]:,.0f}")